...
function playable(state: GameState, type: string): boolean {
  return !state.pending.playedDevThisTurn && state.players[state.currentPlayer].devCards.some((card) => card.type === type && !card.played && card.boughtOnTurn < state.turn);
}

export function mainCandidates(state: GameState, actor: number): Action[] {
  const me = state.players[actor];
  if (state.pending.freeRoads > 0) return legalRoadEdges(state, actor).map((edge) => ({ type: 'buildRoad', edge }));
  ...
  if (playable(state, 'roadBuilding') && legalRoadEdges(state, actor).length > 0) actions.push({ type: 'playRoadBuilding' });
  if (playable(state, 'monopoly')) for (const resource of RESOURCES) actions.push({ type: 'playMonopoly', resource });
  if (playable(state, 'yearOfPlenty')) {
...
function botTradeCandidate(state: GameState, actor: number, difficulty: BotDifficulty): Action | null {
  const me = state.players[actor];
  if (!me.isBot || !state.rules.allowPlayerTrades || state.pending.botTradeOfferedThisTurn) return null;
  const human = state.players.find((player) => !player.isBot);
  if (!human) return null;
  if (difficulty === 'easy' && ((state.rng.seed + state.turn + actor) & 3) !== 0) return null;
  const targets: Partial<ResourceBank>[] = [COSTS.city, COSTS.settlement, COSTS.devCard, COSTS.road];
