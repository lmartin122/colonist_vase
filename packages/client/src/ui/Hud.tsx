...
import { COSTS, RESOURCES, VP_LARGEST_ARMY, VP_LONGEST_ROAD, canAfford, devDeckSize, emptyBank, isConcurrentPhase, longestRoadLength, nextBotAction, publicVictoryPoints, totalResources, victoryPoints } from '@colonist/shared';
        ...
        <div ref={actionPanelRef} className={`relative flex w-full min-w-0 shrink-0 items-stretch justify-start gap-1.5 overflow-x-auto p-1.5 xl:w-auto xl:min-w-[520px] xl:justify-between xl:overflow-visible xl:p-2 ${CARD}`}>
          {myTurn && (canStartRoll || canRoll || game.dice) && (
            ...
            <ActionButton
              ...
              reason={!inMain ? 'Available during your action phase' : devDeckSize(game) === 0 ? 'The development deck is empty' : !canAfford(me.resources, COSTS.devCard) ? 'Not enough resources' : undefined}
            />
            <ActionButton
